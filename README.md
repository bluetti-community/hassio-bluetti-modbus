...
| **FridgePower** | 🧪 Beta, not yet offered | Balco 260's register set on a fridge-sized station: 111 fields, battery sub-device, all read-only until a write has been tested. A real US unit answered the whole Balco 260 profile with values matching the app ([bluetti-registers#38](https://github.com/bluetti-community/bluetti-registers/issues/38)); pack voltage at 0.01 V, signed grid power. Appears in the device dropdown once an owner has confirmed the generated profile in Home Assistant. Modbus TCP is enabled on the unit's local web page. |
