...
class TestAsyncMigrateEntry(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_disables_already_enabled_pv_type_entities_for_ac500(self, er_module):
        ...
        er_module.async_get.return_value = registry
        entity_ids = {
            get_unique_id("entry1 My Device pv_1_i_type"): "sensor.my_device_pv_1_i_type",
            get_unique_id("entry1 My Device pv_2_i_type"): "sensor.my_device_pv_2_i_type",
        }
        registry.async_get_entity_id.side_effect = lambda domain, dom, uid: entity_ids.get(uid)
        registry.async_get.return_value = MagicMock(disabled_by=None)
        hass = MagicMock()
        entry = self._entry(dev_type="ac500")

        result = await async_migrate_entry(hass, entry)

        self.assertTrue(result)
        registry.async_get_entity_id.assert_any_call(
            "sensor", DOMAIN, get_unique_id("entry1 My Device pv_1_i_type")
        )
        registry.async_get_entity_id.assert_any_call(
            "sensor", DOMAIN, get_unique_id("entry1 My Device pv_2_i_type")
        )
        registry.async_update_entity.assert_has_calls(
            [
                call(
                    "sensor.my_device_pv_1_i_type",
                    disabled_by=er_module.RegistryEntryDisabler.INTEGRATION,
                ),
                call(
                    "sensor.my_device_pv_2_i_type",
                    disabled_by=er_module.RegistryEntryDisabler.INTEGRATION,
                ),
            ],
            any_order=True,
    ...
    async def test_does_not_touch_an_already_disabled_pv_type_entity_for_ac500(self, er_module):
        registry = MagicMock()
        er_module.async_get.return_value = registry
        registry.async_get_entity_id.return_value = "sensor.my_device_pv_1_i_type"
        registry.async_get.return_value = MagicMock(disabled_by="user")
        hass = MagicMock()
        entry = self._entry(dev_type="ac500")
    ...
    @patch("custom_components.bluetti_modbus.er")
    async def test_pv_type_not_yet_registered_is_a_no_op_for_ac500(self, er_module):
        registry = MagicMock()
        er_module.async_get.return_value = registry
        registry.async_get_entity_id.return_value = None
        hass = MagicMock()
    ...
    @patch("custom_components.bluetti_modbus.er")
    async def test_non_ac500_device_skips_pv_type_handling(self, er_module):
        # dev_type=balco260 cascades through several earlier, unrelated
        # removal steps (2, 6, 7, 8, 9) too - a blanket entity_id here would
        # make those steps remove a phantom entity, muddying what this test
        # is actually checking. A dict keyed by unique_id, defaulting to
        # None for anything not pv_1_i_type/pv_2_i_type, isolates it: only
        # this step's own lookups matter here, and it must not fire them at
        # all for a dev_type other than ac500.
        registry = MagicMock()
        er_module.async_get.return_value = registry
        entity_ids = {
            get_unique_id("entry1 My Device pv_1_i_type"): "sensor.my_device_pv_1_i_type",
        ...
        # must not disable them for any other dev_type.
        registry.async_update_entity.assert_not_called()
        hass.config_entries.async_update_entry.assert_called_once_with(entry, version=12)
