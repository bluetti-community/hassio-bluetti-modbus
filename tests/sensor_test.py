...
class TestAsyncSetupEntry(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_ac500_field_overrides(self, config_cls, dev_info_fn, get_device_fn):
        ...
        # metadata from field_metadata.py - see const.py's FIELDS_NOT_SHOWN
        # and sensor.py's dataclasses.replace() overrides for the real-
        # hardware evidence behind each one.
        ...
        # g_i_switch (stuck at 1 on real AC500 hardware) and d_serial (this
        # device's identity source instead, see __init__.py) are excluded
        # outright - not even a diagnostic sensor.
        self.assertNotIn("g_i_switch", by_key)
        self.assertNotIn("d_serial", by_key)
